...
function replaceLogo() {
  ...
  if (logoIcons.length > 0) {
    logoIcons.forEach(function (logoIcon) {
      logoIcon.insertAdjacentHTML("afterend", fullLogo);
      logoIcon.remove();
    });
  } else {
...
// Quan sát thay đổi và áp dụng tweaks
function observeChanges(callback) {
  const observer = new MutationObserver((mutations) => {
    mutations.forEach(() => {
      const logo =
        document.querySelector("#logo-icon") ||
        document.getElementsByTagName("ytm-home-logo")[0];
      if (logo) {
        callback();
      }
    });
